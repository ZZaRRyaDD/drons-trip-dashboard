...
class DefaultSettings(BaseSettings):
    ...
    POSTGRES_DB: str = environ.get("POSTGRES_DB", "drons-trip-dashboard")
    ...
    OAUTH2_SCHEME: OAuth2PasswordBearer = OAuth2PasswordBearer(tokenUrl=f"{PATH_PREFIX}/user/authentication")
    ...
    def database_uri(self) -> str:
        ...
        """
        return "postgresql+asyncpg://{user}:{password}@{host}:{port}/{database}?sslmode=disable".format(
    ...
    def database_uri_sync(self) -> str:
        ...
        Get uri for connection with database.
        """
        return "postgresql://{user}:{password}@{host}:{port}/{database}?sslmode=disable".format(
            **self.database_settings,
        )
